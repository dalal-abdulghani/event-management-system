          ...
          <div class="overflow-hidden shadow ring-1 ring-black ring-opacity-5 md:rounded-lg">
            <table class="min-w-full divide-y divide-gray-300">
              ...
              <tbody class="bg-white divide-y divide-gray-200">
                <tr v-for="venue in venuesStore.venues" :key="venue.id">
...
const toggleVenueStatus = async (venue: Venue) => {
  try {
    await venuesStore.updateVenue(venue.id, { is_active: !venue.is_active })
...
const deleteVenue = async (venue: Venue) => {
  if (confirm(`Are you sure you want to delete "${venue.name}"?\n\nThis action cannot be undone.`)) {
    try {
      await venuesStore.deleteVenue(venue.id)
...
const createVenue = async () => {
  try {
    if (editingVenue.value) {
      ...
      await venuesStore.updateVenue(editingVenue.value.id, updatedVenueData)
      ...
      await venuesStore.createVenue(newVenueData)
...
onMounted(async () => {
  try {
    ...
    alert('Failed to load venues from server')
  }
})
</script>
